...
var fnSubject = function fnSubject(n, i, j) {
  fnRel(n) // 값을 전달 받았을때 (ok) 
  .then(function (rel1) {
    return fnRel(i, rel1);
  }).then(function (rel2) {
    return fnRel(j, rel2);
  }).then(function (rel3) {
    var numResult = (rel3 + 5) * 5;
...
// -------------------------------------------------------------------------------
// 커피주문-기기에서 커피내리기-잔에 물/얼음채우기-제품전달
// 녹차주문-녹차티백 열기 - 잔에 물/얼음 채우기 - 제품전달
// id/pw 입력후 , 주소입력표기, 인증요청클릭 후, 인증키처리
// vw 단위계산기 : 값 / 기준 * 100 을 순서대로 처리
// 광고버튼클릭 - 클릭버튼 사라짐 - 화면이동 - 인디케이터 이동 - 클릭버튼 나타남
// 검색입력 후 - 필요한 데이터 찾기 - 관련 data 에서 필요한 값 체크 - 화면에 처리
// -------------------------------------------------------------------------------